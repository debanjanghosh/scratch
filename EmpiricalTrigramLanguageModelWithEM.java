 ...
 */
public class EmpiricalTrigramLanguageModelWithEM implements LanguageModel {
	...
	CounterMap<String, String> trigramValidCounter = new CounterMap<String, String>();
	...
	public void calculateEM ( Collection<List<String>> sentences )
	{
		double lambda_1 = 0.3 ; 
		double lambda_2 = 0.3 ; 
		double lambda_3 = 0.4 ;//lambda1 and lambda2 and lambda3 >=0
		...
		while (true)
		{
			...
			for ( List<String> sentence : sentences )
			{
				...
				String prePreviousWord = words.get(0);
				...
				for (int i = 2; i < words.size(); i++) 
				{
					...
					double trigramProb = getOnlyTrigramProbability(prePreviousWord,previousWord, word);
					double bigramProb = getOnlyBigramProbability(previousWord, word);
					double unigramProb = getOnlyUnigramProbability(word);
				
					double trigramValidCount = getOnlyValidTrigramProbability(prePreviousWord,previousWord, word);
				//	double bigramValidCount = getOnlyValidBigramProbability(previousWord, word);
					...
					double denom = (lambda_1 * trigramProb + lambda_2 * bigramProb +  lambda_3 * unigramProb) ;
				
					c1 +=  ( trigramValidCount * lambda_1 * trigramProb ) / denom ;
					c2 +=  ( trigramValidCount * lambda_2 * bigramProb ) / denom ;
					c3 +=  ( trigramValidCount * lambda_3 * unigramProb ) / denom ;
			...
			double maxLikelihood = computeMaximumLogLikelihood(sentences,lambda_1,lambda_2,lambda_3) ;
		...
		//	if(maxLikelihood > maxmaxLoglike)
			...
			System.out.println("new value of lambda_3" + " " + lambda_3) ;
			
			System.out.println("maxlikelihood is" + "\t" + maxLikelihood+ "\t" + count) ;
			...
			double lambda_1, double lambda_2, double lambda_3)
	{
		...
		for ( List<String> sentence : sentences )
		{
			...
			{
				String word = words.get(i);
				double trigramProb = getTrigramProbability(prePreviousWord,previousWord, word,lambda_1,lambda_2,lambda_3);
				trigramProb = Math.log(trigramProb);
				double trigramValidCount = getOnlyValidTrigramProbability(prePreviousWord,previousWord, word);
				
				logLikeSum += trigramValidCount * trigramProb ;
	...
	public double getOnlyTrigramProbability(String prePreviousWord, String previousWord, String word) 
	{
		double trigramProb = trigramCounter.getCount(prePreviousWord+previousWord, word);
		return trigramProb;
	}
	...
	public double getOnlyValidTrigramProbability(String prePreviousWord, String previousWord, String word) 
	{
		double trigramProb = trigramValidCounter.getCount(prePreviousWord+previousWord, word);
		return trigramProb;
	}
	...
	public double getTrigramProbability(String prePreviousWord,
			String previousWord, String word, double lambda_1, double lambda_2, double lambda_3) 
	{
		double trigramCount = trigramCounter.getCount(prePreviousWord
				+ previousWord, word);
		double bigramCount = bigramCounter.getCount(previousWord, word);
	...
	//		System.out.println("UNKNOWN Word: " + word);
		...
		return lambda_1 * trigramCount + lambda_2 * bigramCount
				+ lambda_3 * unigramCount;
	}
	...
	public EmpiricalTrigramLanguageModelWithEM(
			...
			Collection<List<String>> validationCollection) 
	{
		...
		for (List<String> sentence : validationCollection) 
		{
			...
			for (int i = 1; i < stoppedSentence.size(); i++) 
			{
				...
				trigramValidCounter.incrementCount(prePreviousWord + previousWord,
						word, 1.0);
