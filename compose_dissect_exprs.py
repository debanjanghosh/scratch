...
def createNounVerbSemSpaceFromCore():
    ...
    print "Finding 10 neighbors of \"book-n\""
    neighbors = core_space.get_neighbours("book-n", 10, CosSimilarity())
    print neighbors

def createNounVerbSemSpaceFromSV():
    
    sv_cooccurrence_file  = mainPath + 'sv.sm'
    sv_row_file = mainPath + 'sv.rows'
    sv_col_file = mainPath + 'sv.cols'
    sv_space_file = mainPath + 'sv.pkl'
    print "Building semantic space from sv co-occurrence counts"
    
    sv_space = Space.build(data=sv_cooccurrence_file, rows=sv_row_file,
                                 cols=sv_col_file, format="sm")
                
    print "Applying ppmi weighting"
    sv_space = sv_space.apply(PpmiWeighting())
    print "Applying svd 500"
    sv_space = sv_space.apply(Svd(500))
        
    print "Saving the semantic space"
    io_utils.save(sv_space, sv_space_file)
    
    print "Finding neighbors of \"delivery-n_pay-v\" in the peripheral space"
    #neighbors = sv_space.get_neighbours("delivery-n_pay-v", 10, CosSimilarity())
    neighbors = sv_space.get_neighbours("red-a_wing-n", 10, CosSimilarity())
...
def checkingDummyData():
    
    my_space = Space.build(data = "./data/dissect/input/ex01.sm",
                       rows = "./data/dissect/input/ex01.rows",
                       cols = "./data/dissect/input/ex01.cols",
                       format = "sm")
    
    ppmi_values = my_space.apply(PpmiWeighting()) 

    print ppmi_values
    
    print my_space.cooccurrence_matrix
    io_utils.save(my_space, "./data/dissect/output/ex01.pkl")
    my_space = io_utils.load("./data/dissect/output/ex01.pkl")

    train_data = [("red", "car", "red_car"),
              ("red", "book", "red_book")
              ]
    
    #for ppmi(car,red) = log_e( p(car,red)/p(car) p(red) = log_e ( (5/15)/(8/15) * (6/15) ) = 0.44
...
# use the model to compose words in my_space
    composed_space = my_comp.compose([("red", "book", "red_book")],my_space)
...
if __name__ == '__main__':
   # createNounVerbSemSpaceFromCore()
    createNounVerbSemSpaceFromSV()
