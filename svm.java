...
import com.wsd.kernel.WordEmbedding;
...
abstract class Kernel extends QMatrix 
{
	...
	void swap_index(int i, int j)
	{
		do {wsd_node[] _=x[i]; x[i]=x[j]; x[j]=_;} while(false);
	...
	double kernel_function(int i, int j)
	{
		switch(kernel_type)
		{
			...
			case svm_parameter.WSD:
				return wordEmbeddingObject.wsdKernelInterface(x[i],x[j]) ;
			case svm_parameter.CONTEXT:
				wsd_node[] x1 = getCurrentNodes(x[i]);
				wsd_node[] y1 = getPreviousNodes(x[i]);
				wsd_node[] x2 = getCurrentNodes(x[j]);
				wsd_node[] y2 = getPreviousNodes(x[j]);
				
				double sim = wordEmbeddingObject.wsdKernelInterface(x1,x2) + wordEmbeddingObject.wsdKernelInterface(y1,y2) ;
				return sim ;
	...
	Kernel(int l, svm_node[][] x_, svm_parameter param, WordEmbedding wordEmbeddingObject)
	{
		...
		//loadwordembedding() ;
		loadWordEmbedding(wordEmbeddingObject) ;
		
	}

	public void loadWordEmbedding(WordEmbedding wordEmbeddingObject)
	{
		this.wordEmbeddingObject = wordEmbeddingObject ;
	}
	...
	static double k_function(wsd_node[] x, wsd_node[] y,
					svm_parameter param, WordEmbedding wordEmbeddingObject)
	{
		switch(param.kernel_type)
		{
			...
			case svm_parameter.WSD:
				return wordEmbeddingObject.wsdKernelInterface(x,y);
			case svm_parameter.CONTEXT:
				wsd_node[] x1 = getCurrentNodes(x);
				wsd_node[] y1 = getPreviousNodes(x);
				wsd_node[] x2 = getCurrentNodes(y);
				wsd_node[] y2 = getPreviousNodes(y);
	...
	private static wsd_node[] getCurrentNodes ( wsd_node[] x)
	{
		wsd_node[] x1 = new wsd_node[x.length]; //max length
		int j = 0 ;
		for (int i = 0 ;i < x.length ; i++)
		{
			wsd_node node = x[i] ;
			if ( null == node || node.token == null )
			{
				continue ;
			}
			if (node.token.equalsIgnoreCase("|||"))
			{
				break ;
			}
			else
	...
	private static wsd_node[] getPreviousNodes ( wsd_node[] x)
	{
		int separator = -1 ;
		for ( int i = 0 ; i < x.length ; i++)
		{
		...
		wsd_node[] x1 = new wsd_node[x.length-separator]; //max length
		int j = 0 ;
		for ( int i = separator+1 ; i < x.length ; i++)
		{
			wsd_node node = x[i] ;
			x1[j] = node ;
			j++ ;
...
class SVC_Q extends Kernel
{
	...
	SVC_Q(svm_problem prob, svm_parameter param, byte[] y_, WordEmbedding wordEmbeddingObject)
	{
		super(prob.l, prob.x, param, wordEmbeddingObject);
		y = (byte[])y_.clone();
...
class ONE_CLASS_Q extends Kernel
{
	...
	ONE_CLASS_Q(svm_problem prob, svm_parameter param,WordEmbedding wordEmbeddingObject)
	{
		super(prob.l, prob.x, param,wordEmbeddingObject);
		cache = new Cache(prob.l,(long)(param.cache_size*(1<<20)));
		QD = new double[prob.l];
...
class SVR_Q extends Kernel
{
	...
	SVR_Q(svm_problem prob, svm_parameter param, WordEmbedding wordEmbeddingObject)
	{
		super(prob.l, prob.x, param,wordEmbeddingObject);
		l = prob.l;
...
public class svm {
	...
	private static void solve_c_svc(svm_problem prob, svm_parameter param, WordEmbedding wordEmbeddingObject, 
					...
					double Cp, double Cn)
	{
		...
		Solver s = new Solver();
		s.Solve(l, new SVC_Q(prob,param,y,wordEmbeddingObject), minus_ones, y,
	...
	private static void solve_nu_svc(svm_problem prob, svm_parameter param,
					double[] alpha, Solver.SolutionInfo si, WordEmbedding wordEmbeddingObject)
	{
		int i;
		int l = prob.l;
		double nu = param.nu;
		...
		s.Solve(l, new SVC_Q(prob,param,y,wordEmbeddingObject), zeros, y,
					...
					double[] alpha, Solver.SolutionInfo si, WordEmbedding wordEmbeddingObject)
	{
		...
		Solver s = new Solver();
		s.Solve(l, new ONE_CLASS_Q(prob,param,wordEmbeddingObject), zeros, ones,
			alpha, 1.0, 1.0, param.eps, si, param.shrinking);
	}
					...
					double[] alpha, Solver.SolutionInfo si, WordEmbedding wordEmbeddingObject)
	{
		...
		Solver s = new Solver();
		s.Solve(2*l, new SVR_Q(prob,param,wordEmbeddingObject), linear_term, y,
	...
	static decision_function svm_train_one(
		svm_problem prob, svm_parameter param,WordEmbedding wordEmbeddingObject,
		double Cp, double Cn)
	{
		...
		switch(param.svm_type)
		{
			case svm_parameter.C_SVC:
				solve_c_svc(prob,param,wordEmbeddingObject, alpha,si,Cp,Cn);
				break;
			case svm_parameter.NU_SVC:
				solve_nu_svc(prob,param,alpha,si, wordEmbeddingObject);
				break;
			case svm_parameter.ONE_CLASS:
				solve_one_class(prob,param,alpha,si, wordEmbeddingObject);
				break;
			case svm_parameter.EPSILON_SVR:
				solve_epsilon_svr(prob,param,alpha,si, wordEmbeddingObject);
				break;
			case svm_parameter.NU_SVR:
				solve_nu_svr(prob,param,alpha,si, wordEmbeddingObject);
				break;
		}
	...
	private static void svm_binary_svc_probability(svm_problem prob, svm_parameter param, double Cp, 
			double Cn, double[] probAB, WordEmbedding wordEmbeddingObject)
	{
		int i;
		int nr_fold = 5;
		...
		for(i=0;i<nr_fold;i++)
		{
			...
			subprob.x = new wsd_node[subprob.l][];
			...
			else
			{
				...
				svm_model submodel = svm_train_WSD(subprob,subparam, wordEmbeddingObject);
				for(j=begin;j<end;j++)
				{
					double[] dec_value=new double[1];
					svm_predict_values(submodel,prob.x[perm[j]],dec_value,wordEmbeddingObject);
	...
	private static double svm_svr_probability(svm_problem prob, svm_parameter param, WordEmbedding wordEmbeddingObject)
	{
		int i;
		int nr_fold = 5;
		...
		svm_cross_validation(prob,newparam,nr_fold,ymv,wordEmbeddingObject);
		for(i=0;i<prob.l;i++)
		{
	...
	// Interface functions
	//
	public static svm_model svm_train_WSD(svm_problem prob, svm_parameter param, WordEmbedding wordEmbeddingObject)
	{
		...
		model.param = param;
		{
			...
			for(i=0;i<nr_class;i++)
				for(int j=i+1;j<nr_class;j++)
				{
					...
					sub_prob.l = ci+cj;
					sub_prob.x = new wsd_node[sub_prob.l][];
					...
					if(param.probability == 1)
					{
						...
						svm_binary_svc_probability(sub_prob,param,weighted_C[i],weighted_C[j],probAB, wordEmbeddingObject);
						probA[p]=probAB[0];
						probB[p]=probAB[1];
					}

					f[p] = svm_train_one(sub_prob,param,wordEmbeddingObject, weighted_C[i],weighted_C[j]);
					for(k=0;k<ci;k++)
	...
	public static void svm_cross_validation(svm_problem prob, svm_parameter param, int nr_fold, double[] target,
			WordEmbedding wordEmbeddingObject)
	{
		int i;
		...
		for(i=0;i<nr_fold;i++)
		{
			...
			svm_model submodel = svm_train_WSD(subprob,param, wordEmbeddingObject);
			if(param.probability==1 &&
			    ...
			    param.svm_type == svm_parameter.NU_SVC))
			{
				...
				for(j=begin;j<end;j++)
					target[perm[j]] = svm_predict_probability(submodel,prob.x[perm[j]],prob_estimates,wordEmbeddingObject);
			}
			else
				for(j=begin;j<end;j++)
					target[perm[j]] = svm_predict(submodel,prob.x[perm[j]],wordEmbeddingObject);
		}
	}
	...
	public static double svm_predict_values(svm_model model, wsd_node[] x, double[] dec_values, 
			WordEmbedding wordEmbeddingObject)
	{
		int i;
		   ...
		   model.param.svm_type == svm_parameter.NU_SVR)
		{
			...
			double sum = 0;
			for(i=0;i<model.l;i++)
				sum += sv_coef[i] * Kernel.k_function(x,model.SV[i],model.param,wordEmbeddingObject);
			sum -= model.rho[0];
			dec_values[0] = sum;
		...
		else
		{
			...
			for(i=0;i<l;i++)
				kvalue[i] = Kernel.k_function(x,model.SV[i],model.param,wordEmbeddingObject);
	...
	public static double svm_predict(svm_model model, wsd_node[] x,WordEmbedding wordEmbeddingObject)
	{
		int nr_class = model.nr_class;
		double[] dec_values;
		if(model.param.svm_type == svm_parameter.ONE_CLASS ||
		...
		double pred_result = svm_predict_values(model, x, dec_values,wordEmbeddingObject);
		return pred_result;
	}

	public static double svm_predict_probability(svm_model model, wsd_node[] x, double[] prob_estimates, 
			WordEmbedding wordEmbeddingObject)
	{
		    ...
		    model.probA!=null && model.probB!=null)
		{
			...
			svm_predict_values(model, x, dec_values,wordEmbeddingObject);
			...
			return svm_predict(model, x,wordEmbeddingObject);
	}
	...
	{
		"linear","polynomial","rbf","sigmoid","precomputed", "wsd", "context"
	};
	...
	//	DataOutputStream fp = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(model_file_name)));
		...
		BufferedWriter fp = new BufferedWriter(
			      new OutputStreamWriter(new FileOutputStream(model_file_name), "UTF-8"));
		...
		for(int i=0;i<l;i++)
		{
			...
			else if (param.kernel_type == svm_parameter.WSD || param.kernel_type == svm_parameter.CONTEXT)
			{
	...
	public static svm_model svm_load_model(BufferedReader fp) throws IOException
	{
		...
		for(int i=0;i<l;i++)
		{
			...
			model.SV[i] = new wsd_node[n];
			for(int j=0;j<n;j++)
			{
				model.SV[i][j] = new wsd_node();
				model.SV[i][j].token = st.nextToken();
			}
		}
	...
	public static String svm_check_parameter(svm_problem prob, svm_parameter param)
	{
		...
		if(kernel_type != svm_parameter.LINEAR &&
		   ...
		   kernel_type != svm_parameter.WSD &&
		   kernel_type != svm_parameter.CONTEXT
