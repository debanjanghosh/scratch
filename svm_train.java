...
public class svm_train {
	...
	private String wordEmFile;
	private static final int SHRINKAGE_THRESHOLD = 5000 ;
	private String vocabFile ;
	private HashSet<String> vocabs;
	...
	private void do_cross_validation()
	{
		...
		svm.svm_cross_validation(prob,param,nr_fold,target, wordEmbeddingObject);
	...
	//	read_WSD_problem();
		...
		{
			model = svm.svm_train_WSD(prob,param,wordEmbeddingObject);
	...
	{
		System.out.println("vocab file: " + vocabFile) ;
	...
	//String wordEmFile = "tweet.all.03222015.tokens.lemma.emoji.wmf.model.bin.txt" ;
		...
		//for gensim
		System.out.println("WSD model-input path: " + embedPath) ;
		System.out.println("WSD model-input file: " + wordEmFile) ;
	...
	//	param.kernel_type = svm_parameter.WSD;
		param.degree = 3;
	...
	public void read_WSD_problem() throws IOException
	{
		 BufferedReader fp = new BufferedReader(
			      new InputStreamReader(new FileInputStream(this.input_file_name), "UTF-8"));

		Vector<Double> vy = new Vector<Double>();
		Vector<wsd_node[]> vx = new Vector<wsd_node[]>();
		int max_index = 0;
		...
		while(true)
		{
			...
			vy.addElement(label); //label
			int m = tokens.length-1;
			wsd_node[] x = new wsd_node[m];
			for(int j=1;j<=m;j++)
			{
				x[j-1] = new wsd_node();
				if(tokens[j].toLowerCase().equalsIgnoreCase(param.target) ||
						tokens[j].toLowerCase().equalsIgnoreCase("#"+param.target) )
				{
					continue ;
				}
		...
		fp.close();
		System.out.println("target is: " + param.target) ;
		System.out.println("number of sarcastic training instances is:  " + sarcCount) ;
		System.out.println("number of non-sarcastic training instances is:  " + nonSarcCount) ;
	
		param.cache_size = 2048 ;
	}
	public void read_context_problem() throws IOException
	{
		System.out.println("training file: " + this.input_file_name) ;
		 BufferedReader fp = new BufferedReader(
			      new InputStreamReader(new FileInputStream(this.input_file_name), "UTF-8"));
